...
public class Table_Unternehmen extends SimpleTagSupport {
	public void doTag() {
		...
		List<String> beschreibung = new ArrayList<String>();
		...
		beschreibung = Statements.zeigeDaten("beschreibung", 2);
		...
		String table = "<div class='table-responsive'>"
				...
				+ "			<tr>"
				+ "             <th name='bearbeiten'>Bearbeiten</th>" 
				+ "				<th>Name</th>" 
				+ "				<th>URL</th>" 
				+ "				<th>Nickname</th>" 
				+ "				<th>Aktiv</th>"
		...
		for(int i = 0; i < namen.size();i++) {
			table+="<tr name='row'>"
					+ "<td name='bearbeiten'><input type='radio' onclick=change() name='radio'></td>"
					+ "<td>" + namen.get(i) + "</td>"
					+ "<td>" + url.get(i) + "</td>"
					+ "<td>" + nicknames.get(i) + "</td>"
					+ "<td>" + aktiv.get(i) + "</td>"
					+ "<td>" + beschreibung.get(i) + "</td>"
					+ "</tr>";
		}
		table+="</table>"
				+ "</div>";
		...
		try {
			...
			table+="<script>"
					...
					+ "document.getElementById('beschreibung').value = list[i].cells[5].innerHTML;"
					+ "	}\r\n" + 
					"}"
